...
OGXML::OGXML (
	...
	LOG_IF (FATAL, ! getXMLItem <string> (_sysPath, OGFT_CFGFILE, "path/system") )
		<< "Tag: 'path/system' must be provided";
	_sysDoc.LoadFile (_sysPath.c_str () );
}
...
OGXML::getXMLItem (
	...
	auto								q {OGUtils::splitString (path, '/')};
...
OGXML::goToLastNode (
	...
	switch (file) {
		...
		case OGFT_SYSFILE: elt = _sysDoc.FirstChildElement (); break;
