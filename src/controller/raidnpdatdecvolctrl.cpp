...
//! \file	raidnpdatdecvolctrl.cpp
//! \brief	Definition of the RAID NP controller with data declustering.
...
RAIDNPDatDecVolCtrl::RAIDNPDatDecVolCtrl (
	const Volume			& vol,
	const Device			& dev) {
...
RAIDNPDatDecVolCtrl::decompose (
		...
		while (remainingSize > 0) {
			...
			if (a == 0 && remainingSize >= stripeSize) {
				...
				for (auto y = 0; y < a;) {
					...
					if (_numParity)
						sr._idxDevice = (_numDevices - (s % _numDevices) + sr._idxDevice) % _numDevices;
					sr._type = RQT_READ;
	...
	while (remainingSize > 0) {
		...
		sr._idxDevice = (addr / _numBytesBySU) % (_numDevices - _numParity);
		if (_numParity)
			sr._idxDevice = (_numDevices - (currentStripe % _numDevices) + sr._idxDevice) % _numDevices;
...
void
RAIDNPDatDecVolCtrl::manageFailure (
	Request					& request,
	vector <Request>		& subrequests) {
...
void RAIDNPDatDecVolCtrl::removeDuplicates (
	Request					& request,
	vector <Request>		& subrequests) {
...
OGSS_Bool
RAIDNPDatDecVolCtrl::targetsParity (
	Request					& request) {
...
void
RAIDNPDatDecVolCtrl::updateScheme (
	const Request			& event) {
...
void
RAIDNPDatDecVolCtrl::generateDecraidFailureRequests (
	Request								& block,
	vector <Request>					& subrequests) {
...
void
RAIDNPDatDecVolCtrl::generateDecraidRenewalRequests (
	Request								& block,
	vector <Request>					& subrequests) {
	for (auto i = 0; i < _numDevices; ++i) {
		Request req {block};
...
void
RAIDNPDatDecVolCtrl::generateFailureRequests (
	Request			& block,
	vector <Request>		& subrequests) {  }

void
RAIDNPDatDecVolCtrl::generateRenewalRequests (
	Request								& block,
	vector <Request>					& subrequests) {
	for (auto i = 0; i < _numDevices; ++i) {
		Request req {block};
...
void
RAIDNPDatDecVolCtrl::requestMultiplier (
	Request								& block,
	vector <Request>					& subrequests) {
	Request								sr {block};
...
void
RAIDNPDatDecVolCtrl::getBlockLocation (
	Request					& block) {  }
...
UT_RAIDNPDatDecVolCtrl::UT_RAIDNPDatDecVolCtrl (
	const OGSS_String		& configurationFile):
	UnitaryTest <UT_RAIDNPDatDecVolCtrl> (MTP_VOLRAIDNPPARDEC) {
...
#include "utest/utdatdecp0.cpp"
#include "utest/utdatdecp1.cpp"
#include "utest/utdatdecp2.cpp"
